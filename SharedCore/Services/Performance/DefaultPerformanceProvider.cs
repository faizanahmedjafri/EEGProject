...
{
    public class DefaultPerformanceProvider : IPerformanceProvider
    {
        ...
        readonly ConcurrentDictionary<string, Statistic> _Statistics = new ConcurrentDictionary<string, Statistic>();
        public IDictionary<string, Statistic> Statistics => _Statistics;

        public void Start(string reference, string tag = null, [CallerFilePath] string path = null,
        ...
        public void Stop(string reference, string tag = null, [CallerFilePath] string path = null,
            [CallerMemberName] string member = null)
        {
            string id = GetId(tag, path, member);
        ...
        public string DumpStats()
        {
            ...
            b.AppendLine(
                "ID                                                                                 | Call Count | Total Time | Avg Time");
            foreach (KeyValuePair<string, Statistic> kvp in Statistics.OrderBy(kvp => kvp.Key))
            {
                ...
                b.AppendFormat("{0,-80} | {1,-10} | {2,-10:F2}ms | {3,-8:F2}ms", key, kvp.Value.CallCount, total, avg);
                b.AppendLine();
            }
